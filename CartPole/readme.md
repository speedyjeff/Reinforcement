...
### Neural Network
A Neural Network is a complex equation made up of weights and bias'.  The network is trained with examples generated by the random model which hit a particular threshold (>=50).  These inputs are replayed in the model (except for the last choice, which lead to the random's last run).  The iterations are split (80/20) with the first part being all training and the second part using the model to predict.
